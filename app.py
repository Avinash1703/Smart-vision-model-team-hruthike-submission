...
#     # Create the component with proper iframe communication
...
#             if isinstance(component_value, str) and component_value.startswith('data:image/jpeg;base64,'):
...
# Freshness Detector Page
def freshness_detector_page(model, fruit_vegetable_mapping, freshness_mapping):
    st.markdown('<div class="title">Product Freshness and Shelf Life Estimator</div>', unsafe_allow_html=True)

    # Custom CSS (retain if desired)
    st.markdown("""
        <style>
        /* Your existing CSS */
        </style>
    ...
    if image is not None:
        ...
        if product_type == "Fruit/Vegetable":
            fruit_label, freshness_label = make_prediction(image, model)
            fruit_name = fruit_vegetable_mapping.get(fruit_label, "Unknown")
            freshness = freshness_mapping.get(freshness_label, "Unknown")
            ...
            try:
                ...
                freshness_status = 'Fresh ✅' if info_dict.get('freshness') == 'Yes' else 'Not Fresh ❌'
                shelf_life_days = info_dict.get('shelf_life', 'N/A')
                ...
                insert_freshness_record(
                    produce='Bread',
                    freshness='Fresh' if info_dict.get('freshness') == 'Yes' else 'Not Fresh',
                    expected_life_span_days=shelf_life_days
                )
                st.success("Detection results saved to the database.")
            except Exception as e:
...
def main(json_file_path="data.json"):
    ...
    if page == "Signup/Login":
        ...
        if st.session_state.get("logged_in"):
            model = load_resource()
            freshness_detector_page(model, fruit_vegetable_mapping, freshness_mapping)
        else:
            st.warning('Please Login to use the app!!')
